            ...
            {isSelectProject ? (
                <div className={'sideBar'}>
                    <div className={'sideBarHeader'}>
                        <span className={'sideBarTitle'}>프로젝트 상세 정보</span>
                        <button type={'button'} className={'sideBarCloseBtn'} onClick={handleProjectInfoClose}>
                    ...
                    <div className={'projectInfoSection'}>
                        <div className={'projectInfoHeader'}>
                            ...
                            <div className={'iconBtnSection'}>
                                <button type={'button'} className={'iconBtn iconBtnGreen'}
                                ...
                                <button type={'button'} className={'iconBtn iconBtnRed'} onClick={handleDelete}>
                        ...
                        <div key={isSelectProject.idx}
                             className={'projectInfoContent mt-3'}>
                            <span className={'text-muted small'}>주소</span>
                            <span className={'projectInfoContentTxt'}>
                                {isSelectProject.siteAddress}
                            </span>
                            <span className={'text-muted small'}>과업기간</span>
                            <span className={'projectInfoContentTxt'}>
                                {isSelectProject.startDate} ~ {isSelectProject.endDate}
                            </span>
                            <span className={'text-muted small'}>시공사</span>
                            <span className={'projectInfoContentTxt'}>
                                {isSelectProject.contractor}
                            </span>
                            <span className={'text-muted small'}>계측사</span>
                            <span className={'projectInfoContentTxt'}>
                                {isSelectProject.measurer}
                            </span>
                        </div>
                    </div>
                    <div className={'projectInfoHeader'}>
                        <span className={'projectInfoTitle'}>구간</span>
                    </div>
                        ...
                        <ul className={'sideBarProjectList'}>
                            {sectionList.map((section) => (
                                <li key={section.idx}
                                    ...
                                    <div className={`projectInfoHeader ${selectedSection?.idx === section.idx ? 'selected' : ''}`}>
                                        <button type={'button'} className={'iconBtn secSelectBtn'}
                                    ...
                                    {selectedSection?.idx === section.idx && (
                                        <ul className={`instrumentList ${selectedSection?.idx === section.idx ? 'open' : ''}`}>
                                            {instrumentList.length > 0 ? (
                                                instrumentList.map((instrument, index) => (
                                                    instrument && instrument.insName ? (
                                                        <li key={instrument.idx} className={`instrumentItem ${selectedInstrument?.idx === instrument.idx ? 'active' : ''}`}
                                                            onClick={() => handleSelectInstrument(instrument)}>
                                                            {instrument.insName}
                                                        </li>
                                                    ) : (
                                                        <li key={index} className={'instrumentItem'}>계측기 정보가 잘못되었습니다</li>
                                                    )
                                                ))
                                            ) : (
                                                <li className={'instrumentItem'}>계측기 정보가 없습니다</li>
                                            )}
                                        </ul>
                                    )}
                                </li>
                ...
                <div className={'sideBar'}>
                    <div className={'sideBarHeader'}>
                        <span className={'sideBarTitle'}>현장 리스트</span>
                        <svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" fill="currentColor"
                             className="bi bi-buildings greenIcon" viewBox="0 0 16 16">
                            <path
                    ...
                    <div className={'sideBarContent'}>
                        <ul className={'sideBarProjectList'}>
                            {projectList.map((item) => (
                                <li className={'projectItem'} key={item.idx} onClick={() => handleSelectProject(item)}>
                                    {item.siteName}
                                </li>
                            ))}
                        </ul>
                        <button className={'projectBtn'} type={'button'} onClick={enableDrawing}>
                            {projectBtnText}
                        </button>
                    </div>
                </div>
